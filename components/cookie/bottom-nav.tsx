...
export function BottomNav({ onCreateClick, onDiscoverClick, onMessagesClick, onProfileClick }: BottomNavProps) {
  return (
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 z-50 bg-background/95 backdrop-blur-md border-t border-border/50 pb-safe w-full max-w-md">
      <div className="flex items-center justify-around px-2 py-2">
        {navItems.map((item, index) => {
          ...
          const handleClick =
            item.label === "Discover" ? onDiscoverClick :
            item.label === "Messages" ? onMessagesClick :
            item.label === "Profile" ? onProfileClick :
            undefined
          return (
            <button
              key={item.label}
